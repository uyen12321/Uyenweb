<?php
    class ordermodel extends database{
        ...
        function GetAllOrder(){
            $sql = "SELECT * FROM order_product WHERE cancel_order = 0 ORDER BY 'id'";
        ...
        //Xử Lý đơn hàng
        function orderprocessing($id,$idPersonnel){
            $sql = "UPDATE order_product SET id_personnel = $idPersonnel ,status='Đã Xử Lý' WHERE id = $id";
            $query = $this->conn->prepare($sql);
            $query->execute();
        }
