    ...
    class login extends controller{
        ...
        function admin(){
            if(isset($_COOKIE["user"])){
                $cookie = $_COOKIE["user"];
                $result = $this->commonmodel->GetCookie($cookie,"admin_account");
                $isAdmin = explode(";",$_COOKIE["user"])[2];
                if($result >= 1){
                    if($isAdmin == 1){
            ...
            if($result >= 1 ){
                $infoUser = $this->commonmodel->GetInfoAdmin($user);
                $cookie = randomcookie(150);
                $cookie.=";".$infoUser[0]["id"].";".$infoUser[0]["level_admin"];
